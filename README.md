...
Local replacement failures retry only the same flushed staging file, at most three attempts with 50 ms and 150 ms waits. Persistent failures leave the original destination in place and retain the uniquely named `.tmp` file for diagnosis/recovery; no delete-then-move fallback is used. CLI error reports include the destination, staging path, save operation, attempt count, destination attributes, exception type/HResult, and full local stack. These reports contain local paths but no request bodies or API keys. Review retained files manually; they are not automatically imported or treated as cached responses. A failed save never automatically retries a speech request. Permissions/read-only errors require correcting the underlying filesystem condition; this mitigation does not establish the cause of the reported TellVR incident.
...
#### Asset Tracer - Exact References and Limitations
...
#### Asset Tracer - Review and Cleanup
